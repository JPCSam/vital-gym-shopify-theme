...
(function () {
  ...
  theme.cart = {
    ...
    _updateCart: function (params) {
      return $.ajax(params).then(
        function (cart) {
          let check = false;
          let product = parseInt(getCookie("product"));
  ...
  theme.CartForm = (function () {
    ...
    function CartForm(form) {
      ...
      // this.totalSavedMoney = document.getElementById('totalSavingsP');
      // console.log(this.totalSavedMoney)
    ...
    CartForm.prototype = Object.assign({}, CartForm.prototype, {
      ...
      _parseProductHTML: function (html) {
        ...
        console.log("query");
        console.log(doc.querySelector("#totalSavingsP"));
        if (doc.querySelector("#totalSavingsP") == null) {
          console.log('here');
          return {
          ...
          return {
            items: doc.querySelector(".cart__items"),
            totalSavings: doc.querySelector("#totalSavingsP"),
            discounts: doc.querySelector(".cart__discounts"),
          };
        }
      },
      ...
      cartMarkup: function (html) {
        ...
        if (markup.totalSavings == null) {
          var totalSavedMoney = 0;
        } else {
          var totalSavedMoney = markup.totalSavings.dataset.tsaving;
        }
        ...
        if (totalSavedMoney != 0) {
          document.getElementById("totalSavingsP").innerHTML =
  ...
  function accountDashboard() {
    let accountSlide = $(".account-slider");
    accountSlide.slick({
      infinite: true,
      slidesToShow: 1,
      slidesToScroll: 1,
      ...
      autoplay: true,
      autoplaySpeed: 3000,
      appendDots: $("#shopify-section-account-dashboard .slick-dots-custom"),
      prevArrow: $("#shopify-section-account-dashboard .slick-left"),
      nextArrow: $("#shopify-section-account-dashboard .slick-right"),
    });
    $("#discount-copy").click(function () {
      let copyText = document.getElementById("copy_discount");
      copyText.select();
      copyText.setSelectionRange(0, 99999);
      document.execCommand("copy");
      $(".copied-alert").css("opacity", "1");
    });
    $(".customer-login").click(function (e) {
      $(this).toggleClass("dropdown");
    });
    $(".three-dots-icon").click(function (e) {
    ...
    $(document).on("click", 'form[action="/cart/add"]', function (e) {
      let check = false;
      let product = parseInt(getCookie("product"));
      theme.cart.getCart().done(function (cart) {
        if (cart.items && cart.items.length > 0) {
          cart.items.forEach(function (item) {
            if (product === item.id) {
              check = true;
    ...
    $(document).on("click", ".add-cart-variant", function (e) {
      e.preventDefault();
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          $("body").trigger("added.ajaxProduct");
          document.dispatchEvent(new CustomEvent("ajaxProduct:added"));
          //Show discount code at cart drawer
          var codeCookieValue;
          var getCookieDiscount = function (name) {
            var value = "; " + document.cookie;
            var parts = value.split("; " + name + "=");
            if (parts.length == 2) return parts.pop().split(";").shift();
          };
          codeCookieValue = getCookieDiscount("discount");
          if (!codeCookieValue) {
          }
          if (codeCookieValue) {
            console.log("Discount Code - " + codeCookieValue);
            document.getElementById("discount-message").innerHTML =
              "<div class='show-discount'><p>Discount At Checkout: </p><span>" +
              codeCookieValue +
              "</span></div>";
          }
          // var theUrl = window.location.href+'?discount=';
          // var toRedirect = theUrl + discount;
...
$(document).ready(function () {
    ...
    if (window.matchMedia("(max-width: 1024px)").matches) {
      $(window).scroll(function () {
        ...
        if (
          ...
          $(".add-to-cart.add-to-cart--secondary").length
        ) {
          $(".add-to-cart.add-to-cart--secondary").addClass("fixed");
          $(document.body).bind("DOMSubtreeModified", function () {
            if ($("#dummy-chat-button-iframe").length && !chatLoaded) {
              chatLoaded = true;
              $("#dummy-chat-button-iframe").addClass("up");
            }
          });
          $("#dummy-chat-button-iframe").addClass("up");
        } else {
          $(".add-to-cart.add-to-cart--secondary").removeClass("fixed");
          $("#dummy-chat-button-iframe").removeClass("up");
        }
      });
    }
  }

  // Move online chat with active cart
  $(".product-single__form, #addCartForm").submit(function () {
  ...
  $(
    "a.site-nav__link.site-nav__link--icon.js-drawer-open-cart.js-no-transition, #addCartForm"
  ).click(function () {
    $("#dummy-chat-button-iframe").css({
      transform: "translateX(-450px)",
  ...
  $(".js-drawer-close, main:not(#CartDrawer)").click(function () {
    $("#dummy-chat-button-iframe").css({
      transform: "none",
      transition: "transform 0.5s ease",
    });
...
let chatLoadedDashboard = false;
$(document.body).bind("DOMSubtreeModified", function () {
  if ($("#dummy-chat-button-iframe").length && !chatLoadedDashboard) {
    chatLoadedDashboard = true;

    console.log("chat loaded");
    $("#add-to-cart-dashboard").click(function () {
      $("#dummy-chat-button-iframe").css({
        transform: "translateX(-450px)",
        transition: "transform 1s ease",
      });
